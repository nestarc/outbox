...
function createExplorer(
  handlers: { instance: any; methodMetadata: Record<string, string[]> }[],
) {
...
describe('OutboxExplorer', () => {
  ...
  it('should support multiple handlers for the same event type', () => {
    ...
    const explorer = createExplorer([
      {
        instance: instance1,
        methodMetadata: { handleOrder: ['order.created'] },
      },
      {
        instance: instance2,
        methodMetadata: { handleOrder: ['order.created'] },
  ...
  it('should fail fast on duplicate discovery of the same instance method and event', () => {
    const explorer = createExplorer([
      {
    ...
    expect(() => explorer.onModuleInit()).toThrow(
      'Duplicate outbox handler discovery for FakeHandler.handleOrder and "order.created"',
    );
  });
